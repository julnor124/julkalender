...
const IMAGE_SRC = "/images/grinch1.png";
...
interface GrinchMovieGuessGameProps {
  onSolved?: () => void;
}

const GrinchMovieGuessGame = ({ onSolved }: GrinchMovieGuessGameProps) => {
  ...
  useEffect(() => {
    ...
    if (status === "playing" && guessCount >= MAX_GUESSES) {
      setStatus("revealed");
      setMessage("Det var Grinchen! 🎬");
      return;
    }

    if (status === "solved") {
      setMessage("Det är Grinchen 🍿");
      onSolved?.();
      return;
    }
  ...
  }, [guessCount, imageLoaded, status, onSolved]);
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const isCorrect = TARGET_WORDS.some(
      (word) => normalized === normalizeGuess(word)
    );
      ...
      <main className="relative z-10 mx-auto flex min-h-screen w-full max-w-3xl flex-col items-center px-4 pb-20 pt-16">
        <header className="mb-10 text-center">
          ...
          <p className="mt-3 text-base md:text-lg text-[#F9DADA]/80 max-w-2xl mx-auto">
            Vilken julfilm gömmer sig bakom pixlarna?
          </p>
        </header>
        ...
        <section className="flex w-full flex-col items-center gap-10">
          ...
          {status === "solved" && (
            <p className="mt-4 text-lg font-semibold text-[#ffe89c]">
              🍿 Det stämmer! Filmen är Grinchen.
            </p>
          )}
          {status === "revealed" && (
            <p className="mt-4 text-lg font-semibold text-[#ffe89c]">
              🎬 Du fick se hela bilden – det är Grinchen.
            </p>
          )}
          <p className="text-sm text-[#ffe89c]/80">
            Du har {remainingGuesses}{" "}
            {remainingGuesses === 1 ? "gissning" : "gissningar"} kvar.
          </p>
        </section>
      </main>
    </div>
...
export default GrinchMovieGuessGame;
